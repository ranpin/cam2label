...
def drawBounding(img,img1,fname=" "):
    ...
    cv2.imwrite("results65_15\\"+fname,imgnew)
...
# path_list = os.listdir('imgs48_17')
path_list = os.listdir('imgs65_15')
path_list.sort()    #保证读取顺序
print(path_list)


for pathB in path_list:  
    # pathA=r"imgs48_17"   #根据文件夹名称可能需要修改
    pathA=r"imgs65_15"   #根据文件夹名称可能需要修改
    path=os.path.join(pathA,pathB)
    ...
    with GradCAMPlusPlus(model=model, target_layers=target_layers, use_cuda=False) as cam:
        ...
        for grayscale_cam,tensor in zip(grayscale_cams,input_tensor):
            ...
            drawBounding(grayscale_cam,cv2.cvtColor(rgb_img, cv2.COLOR_RGB2BGR),pathB)#在原图上画框
